...
int GET_BIT_NUM(int bitmap, int *first_bit, int *last_bit)
{
	int bit_num = 0;
	int i;
	...
	for(i = 0; i < MATRIX_DEEP; i++) 
    {
        ...
        {
            if(0 == bit_num && NULL != first_bit) *first_bit = i;
...
int get_checked_num(NODE_INFO_ST *node_db)
{
	...
	for(i = 0; i < MATRIX_NUM; i++) {
		...
		else if (GET_BIT_NUM(node_db[i].bitmap, NULL, &last_bit) == 1) {
...
int get_all_bit_num(NODE_INFO_ST *node_db)
{
	...
	for(i = 0; i < MATRIX_NUM; i++) {
        bit_num += GET_BIT_NUM(node_db[i].bitmap, NULL, NULL);
	}
...
int process_sub_node_db_m1(NODE_INFO_ST* sub_list)
{
    ...
    for(i = 0; i < MATRIX_DEEP; i++) {
        if (GET_BIT_NUM(sub_list[i].bitmap, NULL, NULL) == 2) {
            for (j = 0; j < MATRIX_DEEP; j++) {
...
int process_sub_node_db_m2(NODE_INFO_ST* sub_list)
{
    ...
    for(i = 0; i < MATRIX_DEEP; i++) {
        ...
        if(GET_BIT_NUM(temp_bitmap, NULL, &last_bit) == 1) {
...
int scan_node_db(NODE_INFO_ST *node_db)
{
	int checked_num;
    int bit_num;
...
int try_node_db(NODE_INFO_ST *node_db)
{
    ...
    for(i = 0; i < MATRIX_NUM; i++) 
    {
        ...
        if (GET_BIT_NUM(new_node_db[i].bitmap, &first_bit, &last_bit) == 2)
        {
            // try first_bit 
            new_node_db[i].bitmap = 1 << first_bit;
            new_node_db[i].value = first_bit + 1;
            scan_node_db(new_node_db);

            //check if conflict
            if ( check_node_db(new_node_db) ) 
            {
                printf("%d: catch %d, num %d\n", __LINE__, i, first_bit + 1);
                node_db[i].bitmap = 1 << last_bit;
                node_db[i].value = last_bit + 1;
                clear_cnt++;
            }
            else
            {
                // try last_bit
                memcpy(new_node_db, node_db, MATRIX_NUM*sizeof(NODE_INFO_ST));
                new_node_db[i].bitmap = 1 << last_bit;
                new_node_db[i].value = last_bit + 1;
                scan_node_db(new_node_db);
                
                //check if conflict
                if ( check_node_db(new_node_db) ) {
                    printf("%d: catch %d, num %d\n", __LINE__, i, first_bit + 1);
                    node_db[i].bitmap = 1 << first_bit;
                    node_db[i].value = first_bit + 1;
                    clear_cnt++;
                }
            }
        }
