...
int file_md5(char *file_name, unsigned char md5_str[MD5_STR_LEN])
{
	MD5_CTX md5_calc;
	int fd, ret;
	...
	do {
		...
		if (ret <= 0) break;
		MD5Update(&md5_calc, (unsigned char *)buff, ret);
	} while(1);
	MD5Final(md5_str, &md5_calc);
...
int main(int argc, char **argv)
{
	...
	//输出MD5码
	for(int i=0;i<MD5_STR_LEN;i++) printf("%02x", md5[i]);
	printf("  %s \n", argv[1]);
